/*
  mapa.js
  Pequeña utilidad para inicializar un mapa Leaflet usado en las páginas de ejemplo.
  - Incluye basemaps personalizados de SENER (MapTiler) con fallbacks a CARTO.
  - Configura límites de México para navegación restringida.
  - Añade control de basemaps para cambiar entre estilos.
  - Expone window.MAPA.init(options) y utilidades relacionadas.

  Opciones para init:
    container: 'map' (id)
    center: [lat, lng]
    zoom: number
    includeBasemapControl: true/false (mostrar control de basemaps)
    restrictToMexico: true/false (aplicar límites de México)

  Nota: usa los mismos basemaps personalizados que index.js para consistencia.
*/
...
// Exponer configuración de basemaps para otras páginas (elementos.html)
window.SENER_BASEMAPS = window.SENER_BASEMAPS || {
    mapTilerKeys: {
        personal: 'jAAFQsMBZ9a6VIm2dCwg',
        amigo: 'xRR3xCujdkUjxkDqlNTG'
    },
    styles: {
        'SENER Azul': 'STYLE_ID_AZUL',   // reemplaza por styleId real de index.js
        'SENER Light': 'STYLE_ID_LIGHT',  // reemplaza por styleId real
        'SENER Oscuro': 'STYLE_ID_OSCURO' // reemplaza por styleId real
    },
    defaultStyleName: 'SENER Light',
    // helper opcional para formar la URL de tiles (usa la misma plantilla que en index.js)
    getTileUrl(styleId, key) {
        return `https://api.maptiler.com/tiles/${styleId}/{z}/{x}/{y}.png?key=${key}`;
    }
};